...
def hphob_color(hphob):
    # max value is 1, min values is -1
    hp = hphob.copy()
    # normalize
    hp = hp + 1.0
    hp = hp / 2.0
...
def dm_color(dm):
    ...
    norm_dm = dm
    blue_dm = np.array(norm_dm)
    red_dm = np.array(norm_dm)
    blue_dm[blue_dm < 0] = 0
    red_dm[red_dm > 0] = 0
    red_dm = abs(red_dm)
    red_dm[red_dm > max_val] = max_val
    blue_dm[blue_dm < min_val] = min_val
    red_dm = red_dm / max_val
    blue_dm = blue_dm / max_val
    # red_dm[red_dm>1.0] = 1.0
    # blue_dm[blue_dm>1.0] = 1.0
    green_color = np.array([0.0] * len(dm))
    mycolor = [
        [
            COLOR,
            0.9999 - blue_dm[i],
            0.9999 - (blue_dm[i] + red_dm[i]),
            0.9999 - red_dm[i],
        ]
...
def load_ply(
    ...
    # Draw dm
    if (
        "vertex_dm" in mesh.get_attribute_names()
        and "vertex_nx" in mesh.get_attribute_names()
    ):
        dm = mesh.get_attribute("vertex_dm")
        color_array_surf = dm_color(dm)
        for tri in faces:
            vert1 = verts[int(tri[0])]
            vert2 = verts[int(tri[1])]
            vert3 = verts[int(tri[2])]
    ...
    # Draw plane
    if (
        "vertex_plane" in mesh.get_attribute_names()
        and "vertex_nx" in mesh.get_attribute_names()
    ):
        plane = mesh.get_attribute("vertex_plane")
        #color_array_surf = iface_color(plane)
        color_array_surf = colorDict['orange']
        print(color_array_surf)
        for tri in faces:
            vert1 = verts[int(tri[0])]
            vert2 = verts[int(tri[1])]
            vert3 = verts[int(tri[2])]
