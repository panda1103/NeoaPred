...
from NeoaPred.PepConf.data import residue_constants
from NeoaPred.PepConf.utils import feats
...
def atom_dist_deal(atom1, atom2, dist_threshold=3.5):
    aa_dist = atom1-atom2
    value = 1
...
def compute_mhc_pep_fape(
    pred_frames: Rigid,
    pep_frames: Rigid,
    pep_frames_mask: torch.Tensor,
    mhc_positions: torch.Tensor,
    pep_positions_mask: torch.Tensor,
    length_scale: float,
    l1_clamp_distance: Optional[float] = None,
    eps=1e-8,
) -> torch.Tensor:
    """
        Computes MHC_pep FAPE loss.
    ...
    local_target_pos = pep_frames.invert()[..., None].apply(
        mhc_positions[..., None, :, :],
    )
    error_dist = torch.sqrt(
        torch.sum((local_pred_pos - local_target_pos) ** 2, dim=-1) + eps
    ...
    normed_error = error_dist / length_scale
    normed_error = normed_error * pep_frames_mask[..., None]
    normed_error = normed_error * pep_positions_mask[..., None, :]
    normed_error = torch.sum(normed_error, dim=-1)
    normed_error = (
        normed_error / (eps + torch.sum(pep_frames_mask, dim=-1))[..., None]
    )
    normed_error = torch.sum(normed_error, dim=-1)
    normed_error = normed_error / (eps + torch.sum(pep_positions_mask, dim=-1))
    return normed_error
...
def mhc_pep_backbone_loss(
    traj: torch.Tensor,
    mhc_backbone_rigid_tensor: torch.Tensor,
    mhc_backbone_rigid_mask: torch.Tensor,
    pep_backbone_rigid_tensor: torch.Tensor,
    pep_backbone_rigid_mask: torch.Tensor,
    use_clamped_fape: Optional[torch.Tensor] = None,
    clamp_distance: float = 10.0,
    loss_unit_distance: float = 10.0,
    eps: float = 1e-4,
    **kwargs,
    ...
    mhc_gt_aff = Rigid.from_tensor_4x4(mhc_backbone_rigid_tensor)
    pep_gt_aff = Rigid.from_tensor_4x4(pep_backbone_rigid_tensor)
    fape_loss = compute_mhc_pep_fape(
        pred_aff,
        pep_gt_aff,
        pep_backbone_rigid_mask,
        mhc_gt_aff.get_trans(),
        mhc_backbone_rigid_mask,
        l1_clamp_distance=clamp_distance,
        length_scale=loss_unit_distance,
        eps=eps,
    )
...
def mhc_pep_sidechain_loss(
    ...
    renamed_atom14_gt_exists: torch.Tensor,
    alt_naming_is_better: torch.Tensor,
    mhc_renamed_atom14_gt_positions: torch.Tensor,
    mhc_renamed_atom14_gt_exists: torch.Tensor,
    clamp_distance: float = 10.0,
    length_scale: float = 10.0,
    eps: float = 1e-4,
    **kwargs,
    ...
    renamed_gt_frames = Rigid.from_tensor_4x4(renamed_gt_frames)
    rigidgroups_gt_exists = rigidgroups_gt_exists.reshape(*batch_dims, -1)
    mhc_renamed_atom14_gt_positions = mhc_renamed_atom14_gt_positions.view(
        *batch_dims, -1, 3
    )
    mhc_renamed_atom14_gt_exists = mhc_renamed_atom14_gt_exists.view(*batch_dims, -1)
    fape = compute_mhc_pep_fape(
        sidechain_frames,
        renamed_gt_frames,
        rigidgroups_gt_exists,
        mhc_renamed_atom14_gt_positions,
        mhc_renamed_atom14_gt_exists,
        l1_clamp_distance=clamp_distance,
        length_scale=length_scale,
        eps=eps,
    )
...
def fape_loss(
    ...
    bb_loss = backbone_loss(
        traj=out["sm"]["frames"].to(device),
        **{**batch, **config.backbone},
    )
    sc_loss = sidechain_loss(
        out["sm"]["sidechain_frames"].to(device),
        out["sm"]["positions"].to(device),
...
def mhc_pep_fape_loss(
    out: Dict[str, torch.Tensor],
    m_batch: Dict[str, torch.Tensor],
    p_batch: Dict[str, torch.Tensor],
    config: ml_collections.ConfigDict,
    device,
) -> torch.Tensor:
    bb_loss = mhc_pep_backbone_loss(
        traj=out["sm"]["frames"].to(device),
        mhc_backbone_rigid_tensor = m_batch["backbone_rigid_tensor"],
        mhc_backbone_rigid_mask = m_batch["backbone_rigid_mask"],
        pep_backbone_rigid_tensor = p_batch["backbone_rigid_tensor"],
        pep_backbone_rigid_mask = p_batch["backbone_rigid_mask"],
        **config.backbone,
    )
    sc_loss = mhc_pep_sidechain_loss(
        sidechain_frames = out["sm"]["sidechain_frames"].to(device),
        sidechain_atom_pos = out["sm"]["positions"].to(device),
        rigidgroups_gt_frames = p_batch["rigidgroups_gt_frames"],
        rigidgroups_alt_gt_frames = p_batch["rigidgroups_alt_gt_frames"],
        rigidgroups_gt_exists = p_batch["rigidgroups_gt_exists"],
        renamed_atom14_gt_positions = p_batch["renamed_atom14_gt_positions"],
        renamed_atom14_gt_exists = p_batch["renamed_atom14_gt_exists"],
        alt_naming_is_better = p_batch["alt_naming_is_better"],
        mhc_renamed_atom14_gt_positions = m_batch["atom14_gt_positions"],
        mhc_renamed_atom14_gt_exists = m_batch["atom14_gt_exists"],
        **config.sidechain,
    )
    loss = config.backbone.weight * bb_loss + config.sidechain.weight * sc_loss
    # Average over the batch dimension
...
def compute_plddt(logits: torch.Tensor) -> torch.Tensor:
    ...
    #print(bounds.view(*((1,) * len(probs.shape[:-1])), *bounds.shape))
...
def mhc_pep_distogram_loss(
    ...
    **kwargs,
    ...
    logits_1, logits_2 = torch.split(logits, (no_bins, no_bins), dim=3)
    true_bins = torch.sum(dists > boundaries, dim=-1)

    errors_1 = softmax_cross_entropy(
        logits_1,
        torch.nn.functional.one_hot(true_bins, no_bins),
    )
    errors_2 = softmax_cross_entropy(
        logits_2,
        torch.nn.functional.one_hot(true_bins, no_bins),
    )
    errors = (errors_1 + errors_2)/2
    square_mask = m_pseudo_beta_mask[..., None] * p_pseudo_beta_mask[..., None, :]
...
class MP_Loss(nn.Module): #MHC-Peptide-Loss
    def __init__(self, config):
        super(MP_Loss, self).__init__()
        self.config = config

    def forward(self, mp_outputs, p_outputs, m_batch, p_batch, _return_breakdown=False):
        device = m_batch["aatype"].device
        crop_len = torch.tensor(p_batch["aatype"].shape[-1])
        mp_distogram_loss = mhc_pep_distogram_loss(
                                logits=mp_outputs["distogram_logits"].to(device),
                                m_pseudo_beta=m_batch["pseudo_beta"],
                                p_pseudo_beta=p_batch["pseudo_beta"],
                                m_pseudo_beta_mask=m_batch["pseudo_beta_mask"],
                                p_pseudo_beta_mask=p_batch["pseudo_beta_mask"],
                                **self.config.mp_distogram,
                            )
        mp_fape_loss = mhc_pep_fape_loss(
                            p_outputs,
                            m_batch,
                            p_batch,
                            self.config.mp_fape,
        ...
        loss = mp_distogram_loss * self.config["mp_distogram"].weight + \
               mp_fape_loss * self.config["mp_fape"].weight
        loss = loss * torch.sqrt(crop_len)
        return loss, mp_distogram_loss, mp_fape_loss
    ...
    def forward(self, out, batch, _return_breakdown=False):
        ...
        loss_fns = {
            "distogram": lambda: distogram_loss(
                logits=out["distogram_logits"].to(device),
                **{**batch, **self.config.distogram},
            ),
            "experimentally_resolved": lambda: experimentally_resolved_loss(
                logits=out["experimentally_resolved_logits"].to(device),
            ...
            "plddt_loss": lambda: lddt_loss(
                logits=out["lddt_logits"].to(device),
                all_atom_pred_pos=out["final_atom_positions"].to(device),
                **{**batch, **self.config.plddt_loss},
            ),
            "supervised_chi": lambda: supervised_chi_loss(
                out["sm"]["angles"].to(device),
                out["sm"]["unnormalized_angles"].to(device),
        ...
        if(self.config.tm.enabled):
            loss_fns["tm"] = lambda: tm_loss(
                out["tm_logits"].to(device),
                out['final_affine_tensor'].to(device),
                **{**batch, **self.config.tm},
            )
        ...
        cum_loss = cum_loss * torch.sqrt(crop_len)
        losses["loss"] = cum_loss.detach().clone()
        '''
        crop_len = torch.tensor(batch["aatype"].shape[-1])
        cum_loss = cum_loss * torch.sqrt(crop_len)
        return cum_loss, losses['distogram'], losses['experimentally_resolved'], losses['fape'], losses['plddt_loss'], losses['supervised_chi'], losses['violation']
