...
from NeoaPred.PepConf.config import NUM_RES
from NeoaPred.PepConf.data import residue_constants as rc
...
def squeeze_features(protein):
    ...
    protein["resolution"] = torch.squeeze(protein["resolution"], dim=-1)
    protein["seq_length"] = protein["seq_length"][0]
    return protein
...
def pseudo_beta_fn(aatype, all_atom_positions, all_atom_mask):
    ...
    tmp_gly = is_gly[..., None].repeat([1] * len(is_gly.shape) + [3])
    pseudo_beta = torch.where(
        #torch.tile(is_gly[..., None], [1] * len(is_gly.shape) + [3]),#not support in python 3.6
        tmp_gly,
...
def atom37_to_frames(protein, eps=1e-8):
    ...
    #rots = torch.tile(rots, (*((1,) * batch_dims), 8, 1, 1)) #not support in python 3.6
    rots = rots.repeat(*((1,) * batch_dims), 8, 1, 1)
    rots[..., 0, 0, 0] = -1
    rots[..., 0, 2, 2] = -1
    ...
    #restype_rigidgroup_rots = torch.tile( ##not support in python 3.6
    restype_rigidgroup_rots = restype_rigidgroup_rots.repeat(*((1,) * batch_dims), 21, 8, 1, 1)
