...
def to_pdb(prot: Protein) -> str:
    ...
    for i in range(n):
        ...
        if(chain_index is not None):
            ...
            if(i != n - 1 and chain_index[i + 1] != prev_chain_index and prev_chain_index != 0):
...
def from_prediction(
    ...
    aatype = [features["aatype"][i] for i in range(len(seq_mask)) if(seq_mask[i])]
    return Protein(
        aatype=np.array(aatype),
