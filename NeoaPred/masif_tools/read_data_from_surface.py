...
from NeoaPred.masif_tools.compute_polar_coordinates import compute_polar_coordinates
...
def read_data_from_surface(ply_fn, params):
    ...
    elem[elem<0] = 1e-8
    # k1, k2 are principal curvatures.
    k1 = H + np.sqrt(elem)
    k2 = H - np.sqrt(elem)
    # Compute the shape index 
    diff_k1_k2 = k1-k2
    diff_k1_k2[diff_k1_k2==0] = 1e-8
    si = (k1+k2)/diff_k1_k2
    ...
    if "vertex_iface" in mesh.get_attribute_names():
        ...
        mesh.set_attribute("vertex_iface", iface_labels)
    ...
    vertex_ddc = np.zeros(len(mesh.vertices))
    for vix in range(n):
        # Patch members.
        ...
        vertex_ddc[neigh_vix] = np.mean(ddc)
    ...
    mesh.set_attribute("vertex_ddc", vertex_ddc)
    return mesh, input_feat, rho, theta, mask, neigh_indices, iface_labels, np.copy(mesh.vertices)
...
def compute_shape_complementarity(ply_fn1, ply_fn2, neigh1, neigh2, rho1, rho2, mask1, mask2, params):
    """
        ...
        neighX, rhoX, maskX: (N,max_vertices_per_patch) matrices with the indices of the neighbors,
        the distances to the center and the mask
...
def compute_shape_similarity(ply_fn1, ply_fn2, neigh1, neigh2, rho1, rho2, mask1, mask2, params):
    """
        compute_shape_similarity: compute the shape similarity between all pairs of patches. 
        ply_fnX: path to the ply file of the surface of peptides
        neighX, rhoX, maskX: (N,max_vertices_per_patch) matrices with the indices of the neighbors,
        the distances to the center and the mask

        Returns: vX_ss (2,N,10) matrix with the shape similarity (shape similarity 25 and 50) 
        of each vertex to its nearest neighbor in the other peptides, in 10 rings.
    """
    # Mesh 1
    mesh1 = pymesh.load_mesh(ply_fn1)
    # Normals: 
    ...
    w = params['ss_w']
    iface_filter_value = params['ss_iface_filter']
    radius = params['ss_radius']
    num_rings = 10
    scales = np.arange(0, radius, radius/10)
    scales = np.append(scales, radius)
    ...
    v1_ss = np.zeros((2,len(v1), 10))
    v2_ss = np.zeros((2,len(v2), 10))

    # Find all interface vertices
    kdt = cKDTree(v2)
    d, nearest_neighbors_v1_to_v2 = kdt.query(v1)

    select_idx_v1 = np.arange(0, len(v1))
    # Go through every candidate vertex. 
    for cv1_iiix in range(len(select_idx_v1)):
        cv1_ix = select_idx_v1[cv1_iiix]
        # First shape similarity s1->s2 for the entire patch
        patch_idxs1 = np.where(mask1[cv1_ix]==1)[0]
        neigh_cv1 = np.array(neigh1[cv1_ix])[patch_idxs1]
        # Find the point cv2_ix in s2 that is closest to cv1_ix
        cv2_ix = nearest_neighbors_v1_to_v2[cv1_ix]
        ...
        comp1 = [np.dot(patch_n1[x], patch_n2[neigh_cv1_p][x]) for x in range(len(patch_n1))]
        comp1 = np.multiply(comp1, np.exp(-w * np.square(p_dists_v1_to_v2)))
        # Use 10 rings such that each ring has equal weight in shape similarity
        comp_rings1_25 = np.zeros(num_rings)
        comp_rings1_50 = np.zeros(num_rings)
        ...
        v1_ss[0,cv1_ix,:] = comp_rings1_25
        v2_ss[0,cv2_ix,:] = comp_rings2_25
        v1_ss[1,cv1_ix,:] = comp_rings1_50
        v2_ss[1,cv2_ix,:] = comp_rings2_50
