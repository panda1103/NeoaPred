...
def compute_thetas(plane, vix, verts, faces, normal, neighbors, idx):
    ...
    for i in range(len(idx[vix])):
        ...
        if (len(check_valid) == 3):
            valid = True
            break
        #If the members of the triangle is too far from the vertex (d > radius, so it is not in neighbors),
        #we will modify it with vertex value.
        else:
            tt = tt.copy()
            for x_i in range(len(tt)):
                if(tt[x_i] not in neighbors):
                    tt[x_i] = vix
                    valid = True
    try:
        assert(valid)
    ...
    # Check that if the members of the triangle are repeated.
    check_rep = list(set(tt))
    if (len(check_rep) == 3):
        # Find the two vertices (v1ix and v2ix) in tt that are not vix
        neigh_tt = [x for x in tt if x != vix]
    elif(len(check_rep) == 2):
        neigh_tt = [x for x in tt if x != vix]
        neigh_tt.append(vix)
    elif(len(check_rep) == 1):
        neigh_tt = [x for x in tt if x != vix]
        neigh_tt.append(vix)
        neigh_tt.append(vix)
    else:
        print("Abnormal data. The members of the triangle: ", check_rep)
